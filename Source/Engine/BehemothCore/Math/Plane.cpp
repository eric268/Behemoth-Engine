...
namespace Math
{
	Plane Plane::TransformPlane(const Plane& plane, const Matrix4x4 m)
	{
		Vector4 extendedNormal(plane.normal.x, plane.normal.y, plane.normal.z, 0.0f);
		Vector4 transformedNormal = extendedNormal * m;
		...
		Vector4 transformedPoint = extendedPointOnPlane * m;
